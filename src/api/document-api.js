...
export const DocumentApi = {
    add: async (credential) => {
        try {
            const res = await axiosInstance.post(`${API_URL}${API_ENDPOINTS.DOCUMENT}`, credential);
    ...
    delete: async (credential) => {
        try {
            const res = await axiosInstance.delete(`${API_URL}${API_ENDPOINTS.DOCUMENT}/${credential}`);
