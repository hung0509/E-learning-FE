import { useState } from 'react';
import './discount-detail.css';

const DiscountDetail = ({ discount }) => {
    const [isValid, setIsValid] = useState(discount.isActive === 'VALID');

    const handleValidation = () => {
        setIsValid(!isValid); 
    };

    return (
        <div class="coupon bg-white rounded mb-3 d-flex justify-content-between background-color-common-2">
            <div class="kiri p-3">
                <div class="icon-container ">
                    <div class="icon-container_box">
                        <img src="data:image/png;base64," width="85" alt="totoprayogo.com" class="" />
                    </div>
                </div>
            </div>
            <div class="tengah py-3 d-flex w-100 justify-content-start">
                <div>
                    <span class="badge badge-success text-success">{discount.isActive}</span>
                    <h3 class="lead">{discount.discountCode} </h3>
                    <p class="text-muted mb-0">{discount.discountDescription}</p>
                </div>
            </div>
            <div class="kanan">
                <div class="info m-3 d-flex align-items-center">
                    <div class="w-100">
                        <div class="block">
                            <span class="time font-weight-light">
                                <span style={{ fontSize: '16px' }} className='fw-bold'>{discount.expiredDate}</span>
                            </span>
                        </div>
                        {/* <button type='button' target="_blank" class="btn btn-sm btn-outline-danger btn-block">
                            {discount.isActive}
                        </button> */}
                        <button
                            className={`btn btn-block btn-sm ${isValid ? 'btn-outline-success' : 'btn-outline-danger'}`}
                            disabled={!isValid}
                            onClick={handleValidation}  
                            type='button' target="_blank"
                        > 
                            {discount.isActive}
                        </button>
                    </div>
                </div>
            </div>
        </div>
    );
}

export default DiscountDetail;